...
@fragment fn fs(fsInput: VertexShaderOutput) -> @location(0) vec4f {
  let c1 = textureSample(texture1, sampler1, fsInput.texcoord);
  let c2 = textureSample(texture2, sampler2, fsInput.texcoord);
  if(fsInput.texcoord.x < 0.5) {
      return c1;
  }
  let alpha = 0.5;
  return alpha * c2 + (1-alpha) * c1;
}